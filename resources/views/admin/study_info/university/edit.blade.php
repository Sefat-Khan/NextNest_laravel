<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto bg-white text-black p-6 rounded-xl shadow">
        <h2 class="text-2xl font-bold mb-6">Update University Info</h2>

        <form action="{{ route('universityUpdate', $university->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
...
<!-- Name -->
            ...
            <div>
                ...
                <input type="text" name="area" id="area" value="{{ old('area', $university->area) }}"
                       class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div>
                ...
                <label for="image"
                    ...
                    <span id="mainImageText" class="text-gray-500 {{ $university->image ? 'hidden' : '' }}">Click to upload image</span>
                    <img id="mainImagePreview"
                         src="{{ $university->image ? asset('storage/'.$university->image) : '' }}"
                         class="w-full h-full object-cover rounded-lg {{ $university->image ? '' : 'hidden' }}" />
                </label>
...
<!-- Tag -->
            <div>
                ...
                <input type="text" name="tag" id="tag" value="{{ old('tag', $university->tag) }}"
                       class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
