<x-app-layout>
    ...
    <div class="max-w-4xl mx-auto bg-white text-black p-6 rounded-xl shadow">
        <h2 class="text-2xl font-bold mb-6">Update Scholarship Info</h2>

        <form action="{{ route('scholarshipUpdate', $scholarship->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" id="name" value="{{ old('name', $scholarship->name) }}"
                    ...
                    <select name="university_id" id="university_id"
                        ...
                        @foreach($universities as $university)
                            <option value="{{ $university->id }}" 
                                {{ old('university_id', $scholarship->university_id) == $university->id ? 'selected' : '' }}>
                                {{ $university->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-2">
                ...
                <div>
                    ...
                    <input type="number" name="amount" id="amount" value="{{ old('amount', $scholarship->amount) }}"
                           class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                    ...
                    <input type="date" name="deadline" id="deadline" value="{{ old('deadline', $scholarship->deadline) }}"
                           class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    @error('deadline')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
